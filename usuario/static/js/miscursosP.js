        ...
        if (cursos.hasOwnProperty(cursoKey)) {
            ...
            for (var k = 0; k < curso.horarios.length; k++) {
                ...
                btnEstudiantes.addEventListener('click', (function (subhorario) {
                    return function () {
                        listaEstudiante('lista', selectedPeriodo, subhorario.acta,subhorario);
                    };
                })(subhorarioAux));
                ...
                if (subhorarioAux.nota) {
                    ...
                    divbutton2.className = 'btn-group mr-2 col-md-3 mb-3';
                ...
                if (subhorarioAux.notaExtra) {
                    ...
                    divbutton4.className = 'btn-group mr-2 col-md-3 mb-3';
                    ...
                    // Agregar lógica de clic al botón de actas
                    btnExtr.addEventListener('click', (function (subhorario, botonId) {
                        return function () {
                            notasExtra('Estudiantes' + botonId, botonId, 'notasExtra', selectedPeriodo, subhorario.acta);
                        };
                    })(subhorarioAux, btnExtr.id));
...
function listaEstudiante(tipo, periodo, acta,subs) {
    getData(tipo, periodo, acta)
        .then(function (dataL) {
            ...
            var divTableBoton = document.createElement('div');
            divTableBoton.className = 'row';
            var divTableBotonCol = document.createElement('div');
            divTableBotonCol.className = 'col-12 d-flex gap-2 justify-content-start';

            var btn1 = document.createElement('button');
            btn1.className = 'btn btn-secondary btnPrimaryUC';
            btn1.textContent = 'Firmas';
            btn1.addEventListener("click", function () {
                let data = {
                    periodo: periodo,
                    sede: subs.sede,
                    profesor: subs.profesor,
                    horario: subs.horario,
                    grupo: subs.grupo,
                    curso: subs.curso,
                };
                imprimirExamen(data, acta);
            })

            divTableBotonCol.appendChild(btn1);
            divTableBoton.appendChild(divTableBotonCol);
            ...
            var thCN = document.createElement('th');
            thCN.scope = "col";
            thCN.appendChild(document.createTextNode("#"));
            ...
            data.forEach((estudiante, item) => {
                ...
                td_T2.appendChild(document.createTextNode(' ' + estudiante.telefono2));
                 ...
                 if (estudiante.moroso) {
                    ...
                    td_Co.style.fontWeight = 'bold';
                    td_T1.style.fontWeight = 'bold';
                    td_T2.style.fontWeight = 'bold';
                }

                trb.appendChild(td_Nu);
            ...
            contenedor.appendChild(divTableBoton);
...
async function imprimirExamen(data,acta) {
  var nameE= 'Frimas de estudiantes';
...
function updateTabla(tipo) {
    var contenedor = document.querySelector('#asistenciaName');
    if (contenedor){
        contenedor.remove();
    }
    getData(tipo)
        .then(function (dataL) {
            var data = dataL;
            var rowCa = document.createElement('div');
            rowCa.className = 'row mb-5';
            rowCa.id = 'asistenciaName';
            var colCaBtn = document.createElement('div');
            colCaBtn.className = 'col-md-12 d-flex justify-content-end';
            var divbuttonGroup = document.createElement('div');
            divbuttonGroup.className = 'btn-group';
            divbuttonGroup.role = 'group';
            var buttonPrev = document.createElement('button');
            buttonPrev.className = 'btn btn-primary carousel-control prev-btn carousel-control-prev-icon';
            buttonPrev.type = 'button';
            buttonPrev.setAttribute('data-bs-target', 'carouselBasicExample');
            buttonPrev.setAttribute('data-bs-slide', 'prev');
            buttonPrev.setAttribute('data-direction', 'prev');
            buttonPrev.addEventListener('click', function () {
                var direction = $(this).data("direction");
                $("#carouselBasicExample").carousel(direction);
            });

            var buttonNext = document.createElement('button');
            buttonNext.className = 'btn btn-primary carousel-control next-btn carousel-control-next-icon';
            buttonNext.type = 'button';
            buttonNext.setAttribute('data-bs-target', 'carouselBasicExample');
            buttonNext.setAttribute('data-bs-slide', 'next');
            buttonNext.setAttribute('data-direction', 'next');
            buttonNext.addEventListener('click', function () {
                var direction = $(this).data("direction");
                $("#carouselBasicExample").carousel(direction);
            });

            divbuttonGroup.appendChild(buttonPrev);
            divbuttonGroup.appendChild(buttonNext);

            colCaBtn.appendChild(divbuttonGroup);
            var colCa = document.createElement('div');
            colCa.className = 'col-md-12';
            var carousel = document.createElement('div');
            carousel.id = 'carouselBasicExample';
            carousel.className = 'carousel slide carousel-fade';
            carousel.setAttribute('data-bs-ride', 'carousel');
            var carouselInner = document.createElement('div');
            carouselInner.className = 'carousel-inner';
            carouselInner.id = 'items';
            carousel.appendChild(carouselInner);
            
            data.forEach((estudiantes, index) => {
                var divRowCarrera = document.createElement('div');
                divRowCarrera.className = 'row';
                var divColCarrA = document.createElement('div');
                divColCarrA.className = 'col-md-7 mb-3';
                var spanCarrA = document.createElement('span');
                spanCarrA.id = 'idAsis';
                spanCarrA.innerText = estudiantes.idAsistencia;
                divColCarrA.appendChild(spanCarrA);
                var divColCarr = document.createElement('div');
                divColCarr.className = 'col-md-7 mb-3';
                var spanCarr = document.createElement('span');
                spanCarr.innerText = estudiantes.curso;
                divColCarr.appendChild(spanCarr);
                var divColHor = document.createElement('div');
                divColHor.className = 'col-md-7 mb-3';
                var spanHorr = document.createElement('span');
                spanHorr.id = 'horarioAsi';
                spanHorr.innerText = estudiantes.horario;
                divColHor.appendChild(spanHorr);
                divRowCarrera.appendChild(divColCarrA);
                divRowCarrera.appendChild(divColCarr);
                divRowCarrera.appendChild(divColHor);
                ...
                var table = document.createElement('table');
                table.className = 'table table-striped table-bordered dataTable miTabla';
                table.id = 'tablaLista' + index;
                var thead = document.createElement('thead');
                var tr = document.createElement('tr');
                ...
                tbody.id = 'listBody';
                table.appendChild(tbody);
                var divRowButtonA = document.createElement('div');
                divRowButtonA.className = 'row';
                var divColButtonA = document.createElement('div');
                divColButtonA.className = 'col-md-6';
                var buttonSaveA = document.createElement('button');
                buttonSaveA.id = 'subirAsistencia';
                buttonSaveA.className = 'btn btn-primary';
                buttonSaveA.innerText = 'Subir asistencia';
                buttonSaveA.addEventListener('click', function () {
                    subirAsistencia('tablaLista' + index);
                });
                divColButtonA.appendChild(buttonSaveA);
                divRowButtonA.appendChild(divColButtonA);
                divTable.appendChild(table);
                carouselItem.className = 'carousel-item rounded-3';
                ...
                carouselItem.appendChild(divRowCarrera);
                carouselItem.appendChild(divTable);
                carouselItem.appendChild(divRowButtonA);
            ...
            rowCa.appendChild(colCaBtn);
            rowCa.appendChild(colCa);

            var contenedor = document.querySelector('#modal_funcionesBody');
            contenedor.appendChild(rowCa);
            $('.miTabla').DataTable({
                language: {
                    "sLengthMenu": "Mostrar _MENU_ registros",
                    "sInfo": "Mostrando registros del _START_ al _END_ de un total de _TOTAL_ registros",
    ...
    getData(tipo, periodo, acta)
        .then(function (dataL) {
            ...
            if (document.getElementById(id)) {
                ...
                liError.innerText = 'Valores invalidos, solo se permite valores entre 0 a 100 y NSP, NPF, MAT';
                ...
                data.forEach((estudiante, item) => {
                    ...
                    inputP1.value = estudiante.estado_nota_Primera_Evaluacion === 'CN' ? (estudiante.nota_Primera_Evaluacion ?? '') : (estudiante.estado_nota_Primera_Evaluacion ?? '');
                    ...
                    inputP2.value = estudiante.estado_nota_Segunda_Evaluacion === 'CN' ? (estudiante.nota_Segunda_Evaluacion ?? '') : (estudiante.estado_nota_Segunda_Evaluacion ?? '');
                    ...
                    inputP3.value = estudiante.estado_nota_Tercera_Evaluacion === 'CN' ? (estudiante.nota_Tercera_Evaluacion ?? '') : (estudiante.estado_nota_Tercera_Evaluacion ?? '');
                    ...
                    inputP4.value = estudiante.estado_nota_Otros_Copnceptos === 'CN' ? (estudiante.nota_Otros_Copnceptos ?? '') : (estudiante.estado_nota_Otros_Copnceptos ?? '');
                ...
                btnParcial1.addEventListener('click', function(){
                    subirNota('Parcial1', periodo, acta);
                });
                ...
                btnParcial2.addEventListener('click', function(){
                    subirNota('Parcial2', periodo, acta);
                });
                ...
                btnParcial3.addEventListener('click', function(){
                    subirNota('Parcial3', periodo, acta);
                });
                ...
                btnParcial4.addEventListener('click', function(){
                    subirNota('Parcial4', periodo, acta);
                });
...
function validarData(id, idI, tipo, nombre) {
    ...
    var textRegex = /^(100(\.0+)?|([0-9]{1,2})(\.[0-9]+)?)$/;

    // Expresión regular para validar las opciones "NS", "DF" y "GT"
    var optionRegex = /^(NSP|NPF|MAT)$/;
...
function validarDataEst(id, idI, tipo, nombre) {
    ...
    if (optionRegex.test(inputValue)) {
        ...
        document.getElementById('textoModalDEs').innerText = 'Los datos permitidos son: NSP, NSPF y MAT';
...
function actas(tipo, periodo, acta, sede,profesor,horario,grupo,curso,notaMin) {
    getData(tipo, periodo, acta)
        .then(function (dataL) {
            ...
            btnImprimirActa.style.display = 'none';
            ...
            buttonEnviar.innerText = 'Enviar acta';
            ...
            divColBot.appendChild(buttonEnviar);
            ...
            btnImprimirActa.addEventListener('click', function () {
                ...
                buttonEnviar.style.display = 'inline-block';
                let data = {}
...
function calcularNota() {
    ...
    if (porcentaje1 < 0 || porcentaje1 > 100 || porcentaje2 < 0 || porcentaje2 > 100 || porcentajeFinal < 0 || porcentajeFinal > 100 || porcentajeOtras < 0 || porcentajeOtras > 100) {
    ...
    table.rows().every(function (rowIdx) {
        ...
        var notaFinal = (nota1 * porcentaje1 / 100) + (nota2 * porcentaje2 / 100) + (nota3 * porcentajeFinal / 100) + (nota4 * porcentajeOtras / 100);
...
function subirAsistencia(id){
    console.log(id)
    var activeItem = document.querySelector('.carousel-item.active');
    var firstChild = activeItem.firstElementChild;
    var table = $('#'+id).DataTable();
    var asistencia = {
        "periodo": document.getElementById('periodos').value,
        "asistencia": firstChild.querySelector('#idAsis').textContent,
        "horarioLine_id": firstChild.querySelector('#horarioAsi').textContent,
        "identificacion": "604150895",
        "semana": document.getElementById('selectSemana').value,
        "estudiantes": []
    };
    table.rows().every(function () {
        var row = this;
        var rowData = row.data();
        var checkbox = $(row.node()).find('input[type="checkbox"]')[0];
        var estudiante = {
            "identificacion": rowData[1],
            "estudiante": rowData[2],    
            "presente": checkbox.checked 
        };
        asistencia.estudiantes.push(estudiante);
    });
    data = JSON.stringify(asistencia)
    document.getElementById('textoModalD').innerText = '¿Realmente quieres guardar la asistencia? Este proceso no se puede deshacer.';
    $('#myModal').modal('show');
...
function subirNota(tipo, periodo, acta){
    ...
    newButton.parentNode.replaceChild(newButtonClone, newButton);
    newButtonClone.addEventListener('click', function () {
        // Call the corresponding function based on the 'tipo' parameter
        if (tipo === 'Parcial1') {
            // Call the function for Parcial1
            sendNotas('Parcial', periodo, acta);
        } else if (tipo === 'Parcial2') {
            // Call the function for Parcial2
            sendNotas('Parcial', periodo, acta);
        } else if (tipo === 'Parcial3') {
            // Call the function for Parcial3
            sendNotas('Parcial', periodo, acta);
        } else {
            // Call the function for other cases
            sendNotas('Parcial', periodo, acta);
        }
        // Close the modal
        $('#myModal').modal('hide');
...
function sendNotas(tipo, periodo, acta){
    let notas = guardarNotas(tipo, periodo, acta);  // objeto aún no convertido en string
...
function sendAsistencia(data){
    const url = '/enviarERP/?tipo=asistencia';
    const xhr = new XMLHttpRequest();
    xhr.open('POST', url, true);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
...
function calcularNotaFinal(notaApro){
    var table = $('#tablaLista').DataTable();
    table.rows().every(function (rowIdx) {
        var row = this;
        var rowData = row.data();
        const notaExt =parseFloat(rowData[5]);
        const notaInt = parseFloat(rowData[4]);
        let notaFinal = 0;
        if(notaExt){
            if (notaExt >= notaApro) {
                notaFinal = notaApro
